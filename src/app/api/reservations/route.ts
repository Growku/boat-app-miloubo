...
export async function DELETE(request: NextRequest) {
  ...
  const existing = db.prepare(
    "SELECT id, date FROM reservations WHERE id = ? AND user_id = ? AND cancelled_at IS NULL"
  ).get(Number(id), Number(userId)) as { id: number; date: string } | undefined;
  ...
  const cancel = db.transaction(() => {
    ...
    db.prepare(
      "INSERT INTO activity_log (user_id, action, target_date, detail) VALUES (?, 'cancelled', ?, 'Cancelled reservation')"
    ).run(Number(userId), existing.date);
  });
  cancel();
